...
public class Grapple : MonoBehaviour
{
    ...
    public float grappleLenght = 7;
    public float grappleInLenght = 0.5f;
    ...
    private void FixedUpdate()
    {
        ...
        {
            case eMode.gOut:
                p1 += directions[facing] * grappleSpeed * Time.fixedDeltaTime;
                grapHead.transform.position = p1;
                grapLine.SetPosition(1, p1);
                ...
                if ((p1-p0).magnitude >= grappleLenght)
                {
                    mode = eMode.gInMiss;
                }
                break;
            case eMode.gInMiss:
                p1 -= directions[facing] * 2 * grappleSpeed * Time.fixedDeltaTime;
                ...
                break;
            case eMode.gInHit:
                float dist = grappleInLenght + grappleSpeed * Time.fixedDeltaTime;
                if (dist > (p1 - p0).magnitude)
                {
                    p0 = p1 - (directions[facing] * grappleInLenght);
                    transform.position = p0;
                    StopGrapple();
                    break;
                }
                p0 += directions[facing] * grappleSpeed * Time.fixedDeltaTime;
                transform.position = p0;
                grapLine.SetPosition(0, p0);
                grapHead.transform.position = p1;
                break;
